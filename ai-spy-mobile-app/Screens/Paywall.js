...
export default function Paywall({navigation}) {
  ...
  const GlassSpinner = ({ visible, message = "Loading..." }) => {
    ...
    return (
      <View style={[tw`absolute inset-0 z-50 justify-center items-center`, { backgroundColor: 'rgba(0, 0, 0, 0.7)' }]}>
        <GlassCard style={tw`items-center py-8 px-12`} intensity={40}>
          ...
          <Text style={tw`text-[17px] font-medium text-white text-center`}>
            {message}
          </Text>
  ...
  async function handleMonthlyPurchase(){
    ...
    try {
      ...
      console.log('✅ Purchase completed successfully:', {
        ...
        hasActiveEntitlements: purchaseResult.customerInfo.entitlements.active && 
      ...
      if(purchaseResult.customerInfo.entitlements.active && 
        ...
        Alert.alert("Purchase Completed", "Your subscription is being processed. You should have access shortly.")
        navigation.navigate("TestHome")
      }
    } catch (error) {
      ...
      if (error.code === 'PURCHASE_CANCELLED') {
        ...
        try {
          ...
          console.log('⚠️ Failed to refresh subscription status for existing subscriber:', refreshError)
        }
        ...
        Alert.alert("Payment Pending", "Your payment is pending approval. You'll receive access once it's approved.")
        ...
        Alert.alert("Product Unavailable", "This subscription is not available in your region or there's a configuration issue.")
      } else {
    ...
    return (
      <GlassContainer style={tw`flex-1`}>
        <View style={tw`items-center py-10`}>
          <ActivityIndicator size="large" color="#FFA500" />
          <Text style={tw`text-base font-light text-white/70 mt-4 text-center`}>Loading subscription options...</Text>
        </View>
      </GlassContainer>
    )
  }
  ...
  return (
    <GlassContainer style={tw`flex-1 `}>
      ...
      <ScrollView 
        contentContainerStyle={tw`pt-20 px-5 pb-10`}
      ...
      >
        ...
        <GlassCard style={tw`mb-2 mt-1`}>
          ...
          >
            <View style={tw`w-15 h-15 rounded-full overflow-hidden mr-4 border border-orange-500/20`}>
              <LinearGradient
            ...
            <View style={tw`flex-1`}>
              <Text style={tw`text-base font-semibold text-white mb-1`}>Full Audio Transcription</Text>
              <Text style={tw`text-sm font-light text-white/70 leading-5`}>Get complete transcripts with timestamps and AI risk highlighting</Text>
            </View>
          </MotiView>
          ...
          >
            ...
            <View style={tw`flex-1`}>
              <Text style={tw`text-base font-semibold text-white mb-1`}>Complete Content Analysis</Text>
          ...
          >
            {currentOffering && currentOffering.monthly && (
              <GlassCard style={tw`mb-1`}>
                <View style={tw`items-center `}>
                  <Text style={tw`text-lg font-bold text-white text-center mb-2`}>Premium Subscription</Text>
                  <Text style={tw`text-lg font-extrabold text-orange-400 text-center mb-1`}>
                    {currentOffering.monthly?.product.priceString}/Month
                  </Text>
                  <Text style={tw`text-base font-light text-white/70 text-center mb-2 italic`}>Cancel Anytime</Text>
            ...
            <View style={tw`items-center `}>
              <TouchableOpacity onPress={handleRestorePurchases} style={tw` mt-2 px-4`}>
                <Text style={tw`text-base font-light text-white/70 text-center`}>Restore Purchases</Text>
              </TouchableOpacity>
              ...
              {Platform.OS === 'android' && (
                <>
                  <TouchableOpacity 
                    ...
                    style={tw`py-3 px-5`}
                  >
                    <Text style={tw`text-base font-light text-white/70 text-center`}>How to Redeem an Offer Code</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity 
                    ...
                    style={tw`py-3 px-5`}
                  >
                    <Text style={tw`text-base font-light text-white/70 text-center`}>Enter Organization Code</Text>
                  </TouchableOpacity>
                </>
              )}
            </View>
