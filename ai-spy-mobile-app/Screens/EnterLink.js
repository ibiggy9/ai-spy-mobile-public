...
import { AppState, Text, TouchableOpacity, View, useWindowDimensions, Image, Linking, ScrollView, Alert, TextInput, ActivityIndicator, Clipboard, BackHandler } from 'react-native';
...
export default function EnterLink({navigation}) {
    ...
    function handleProcessingComplete(response, transcription = null) {
      ...
      if (response.Results?.chunk_results) {
        ...
        chunkResults = response.results.map((item, index) => ({
          ...
          Probability_ai: item.prediction === 'ai' ? 
    ...
    async function runPredictionEnhanced() {
      ...
      try {
        ...
        if (error.message && (error.message.includes('HTTPS required') || error.message.includes('426'))) {
          ...
          try {
            ...
            setloading(false);
            setDisplayError('Both primary and fallback services are currently unavailable. Please try again later.');
            setProcessingProgress(0);
            setProcessingStage('starting');
            return;
          }
      ...
      useEffect(() => {
        ...
        const subscription = AppState.addEventListener('change', nextAppState => {
          if (
            ...
            console.log('📱 EnterLink: App came to foreground, checking for pending jobs in 1 second...');
            setTimeout(() => {
              resumePendingJobs();
            }, 1000); // 1 second delay
          }
