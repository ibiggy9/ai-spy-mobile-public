...
import { AppState, Text, TouchableOpacity, View, useWindowDimensions, Image, Linking, ActivityIndicator, ScrollView, Alert, SafeAreaView, Modal } from 'react-native';
...
const LoadingAnimations = React.memo(() => (
  <View style={tw`items-center`}>
    ...
    <Text style={tw`text-[24px] font-semibold text-white text-center mb-8`}>
      Processing Audio
    </Text>
      ...
      try {
        ...
        if (predictionChunks?.Results?.chunk_results) {
          ...
          chunkResultsForReport = predictionChunks.results.map((item, index) => ({
            ...
            Probability_ai: item.prediction.toLowerCase() === 'ai' ? 
          ...
          try {
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
              ...
              try {
                ...
                setloading(false);
                setDisplayError(`Failed to start processing: ${processingError.message || processingError}`);
              }
            } else {
    ...
    <GlassCard style={tw` flex-1 mt-28 mb-10`} intensity={30}>
      ...
      <View style={tw`items-center mt-4`}>
        <View style={tw`flex-row items-center mb-3`}>
          ...
          <Text style={tw`text-[15px] font-medium text-white/70 ml-2`}>
    ...
    >
          ...
          <GlassCard style={tw`flex-1 mt-28 mb-10`} intensity={25}>
            <View style={tw`items-center`}>
              ...
              <Text style={tw`text-[15px] font-normal text-white/60 text-center mb-8 px-4 leading-6`}>
                {displayError || uploadError || "An unexpected error occurred while processing your file"}
              </Text>
              
              <GlassButton 
                onPress={clearFile}
                variant="secondary"
                style={tw`min-w-[150px] mb-4`}
              >
                Try Again
              </GlassButton>
            </View>
      ...
      const renderPredictionResult = () => {
        ...
        return (
          ...
          >
            <LinearGradient
              colors={getPredictionColor(prediction)}
              style={tw`flex-1`}
            >
              <View style={tw`flex-1 justify-center items-center px-8`}>
                <MotiView
                  ...
                  style={tw`mb-8`}
                >
                  <View style={tw`bg-white/10 rounded-full p-8`}>
                ...
                <GlassHeader 
                  ...
                  subtitle={prediction?.toLowerCase() === 'human' ? 'Human Voice Detected' : 
                           prediction?.toLowerCase() === 'ai' ? 'AI Voice Detected' : 
