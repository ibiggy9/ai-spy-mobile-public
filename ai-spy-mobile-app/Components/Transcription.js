...
export default function Transcription({ 
  ...
  useEffect(() => {
    ...
    if (transcriptionData) {
      ...
      console.log("Words array:", Array.isArray(transcriptionData.words) ? `${transcriptionData.words.length} words` : "missing or invalid");
  ...
  const getColorForTimestamp = (timestamp) => {
    ...
    if (result.ai_probability !== undefined) {
      ...
      console.log(`📊 Calculated: isPredictionAI=${isPredictionAI}, confidence=${result.confidence}, aiProbability=${aiProbability}, aiProbabilityPercent=${aiProbabilityPercent.toFixed(1)}%`);
    }
    
    // Debug logging for color calculation
    console.log(`🎨 Color Decision - Timestamp: ${timestamp}, Prediction: ${result.prediction}, AI Probability: ${aiProbabilityPercent.toFixed(1)}%`);
  ...
  const colorizedWords = useMemo(() => {
    ...
    if (validWords.length < transcriptionData.words.length) {
      console.log(`Filtered out ${transcriptionData.words.length - validWords.length} invalid word objects`);
    }
  ...
  const handleUpgradeClick = () => {
    Alert.alert(
      'Upgrade to Pro',
      'Get full transcription access with Pro subscription',
      [
        ...
        <Text style={tw`text-base leading-7`}>
          {displayWords.map((word, index) => (
            <Text 
              key={index} 
              style={[
                getTextColor(word.colorClass),
                index === 0 ? tw`capitalize` : {}
              ]}
            >
              {word.word}{' '}
            </Text>
          ...
          {shouldTruncate && !isExpanded && (
            <Text style={tw`text-white/60`}>...</Text>
          )}
