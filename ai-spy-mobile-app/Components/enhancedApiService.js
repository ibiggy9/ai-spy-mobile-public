...
class EnhancedApiService {
  ...
  async getAuthToken() {
    try {
      ...
      throw new Error('Failed to obtain authentication token. Please check your internet connection and try again.');
    }
  }
  ...
  async getCurrentUserId() {
    try {
      ...
      if (decoded.includes('|')) {
        ...
        if (parts.length !== 4) {
          console.error('Invalid pipe token format - expected 4 parts, got:', parts.length, 'Decoded:', decoded);
          return null;
        }
        ...
        const parts = decoded.split(':');
        if (parts.length < 4) {
          console.error('Invalid colon token format - expected at least 4 parts, got:', parts.length, 'Decoded:', decoded);
          return null;
        }
  ...
  async makeAuthenticatedRequest(url, options = {}, retryCount = 0) {
    ...
    try {
      ...
      if (!fullUrl.startsWith('https://')) {
        ...
        throw new Error('HTTPS is required for all API requests. Ensure your API_BASE_URL uses https://');
      }
  ...
  async getEnhancedJobStatus(jobId, userSubscriptionStatus = null, includeEmbeddings = false) {
    try {
      ...
      return {
        ...
        error: data.error,
        progress_message: data.status === 'pending' ? 'Processing in queue...' : 
  ...
  async checkEmbeddingsAvailability(jobId) {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Enhanced API: Chat embeddings endpoint failed, falling back to job status:', chatError.message);
        ...
        if (lightweightStatus.result?.embeddings_info) {
          return {
            ...
            chunks_with_embeddings: lightweightStatus.result.embeddings_info.chunks_with_embeddings || 0,
        ...
        if (chunkResults.length > 0) {
          const chunksWithEmbeddings = chunkResults.filter(chunk => 
  ...
  async submitEnhancedFileJob(fileUri, fileName, userSubscriptionStatus = null) {
    try {
      ...
      try {
        ...
        console.log('⚠️ Signed URL generation failed with exception, falling back to direct analyze endpoint');
        ...
        const formData = new FormData();
        formData.append('file', new File([fileBlob], fileName || 'audio.mp3', { type: 'audio/mpeg' }));
      ...
      if (!urlResponse.ok) {
        ...
        console.log('⚠️ Signed URL generation failed with status code, falling back to direct analyze endpoint');
  ...
  async submitAndMonitor(linkOrFileUri, userId, callbacks = {}, userSubscriptionStatus = null, isFile = false, fileName = null) {
    try {
      ...
      console.log(`🚀 Using ${hasSubscription ? 'PRO' : 'FREE'} tier for ${isFile ? 'FILE' : 'LINK'} processing`);
  ...
  async sendChatMessage(message, context = null, taskId = null, analysisData = null, hasSubscription = false) {
    try {
  ...
  async getChatEmbeddings(taskId) {
    try {
      ...
      console.log('📊 Retrieved embeddings data:', {
        ...
        chunksWithEmbeddings: data.complete_chunk_data?.filter(chunk => chunk.embeddings && Array.isArray(chunk.embeddings)).length || 0
      });
