...
export default function Results({ 
  ...
  const processedData = React.useMemo(() => {
    ...
    if (result?.Results?.chunk_results && Array.isArray(result.Results.chunk_results) && result.Results.chunk_results.length > 0) {
    ...
    const calculateSummaryStats = (data) => {
      ...
      if (result?.Total_AI !== undefined && result?.Total_Human !== undefined && result?.Total_Clips !== undefined) {
  ...
  useEffect(() => {
    const shadowColors = {
      ...
      'Mixed': { shadowColor: '#a855f7', shadowOpacity: 0.7, shadowRadius: 15, elevation: 15 },
      'contains some ai': { shadowColor: '#a855f7', shadowOpacity: 0.7, shadowRadius: 15, elevation: 15 }
    };
      ...
      >
        ...
        >
          ...
          <View style={tw`mb-4`}>
            ...
            <Text style={tw`text-white/90 text-sm mb-1`}>• Full audio transcription with timestamps</Text>
            <Text style={tw`text-white/90 text-sm mb-1`}>• Advanced content analysis & summaries</Text>
          ...
          >
            <Text style={tw`text-white/70 text-center text-sm`}>
              Starting at {currentOffering?.availablePackages?.[0]?.product?.priceString || currentOffering?.monthly?.product?.priceString || '$9.99'}/month • Cancel anytime
            </Text>
          </TouchableOpacity>
        </LinearGradient>
      </MotiView>
  ...
  const renderTranscription = () => {
    ...
    const aiDetectionResults = processedData.aiDetectionTimeline.length > 0 ? processedData.aiDetectionTimeline : processedData.chartData;
      ...
      <View>
        <GlassCard style={tw`rounded-2xl mb-2`}>
          ...
          <View style={tw`flex-row border-b border-white/20 p-3`}>
            ...
            <Text style={tw`text-white/80 text-sm font-semibold flex-1 text-center`}>AI Detection</Text>
            <Text style={tw`text-white/80 text-sm font-semibold flex-1 text-right`}>Confidence</Text>
          </View>
          ...
          <ScrollView style={tw`max-h-80`} nestedScrollEnabled={true}>
            {displayData.map((dataPoint, index) => {
              ...
              const predictionCategory = getPredictionCategory(dataPoint.prediction, dataPoint.probability_ai);
          ...
          {selectedPoint && (
            <GlassCard style={tw`mt-4`}>
              <Text style={tw`text-white font-semibold mb-2`}>Chunk {selectedPoint.chunk} Details:</Text>
              <Text style={tw`text-white/80 text-sm`}>• Prediction: {selectedPoint.prediction.toUpperCase()}</Text>
              <Text style={tw`text-white/80 text-sm`}>• AI Probability: {selectedPoint.probability_ai.toFixed(1)}%</Text>
              <Text style={tw`text-white/80 text-sm`}>• Model Confidence: {selectedPoint.confidence.toFixed(1)}%</Text>
              <Text style={tw`text-white/80 text-sm`}>• Time: {formatTime(selectedPoint.timestamp)} - {formatTime(selectedPoint.timestamp + 3)}</Text>
                ...
                {isSubscriptionLoading ? (
                  ...
                  <View style={tw`bg-black/30 rounded-lg p-3 border border-orange-500/30`}>
                    <View style={tw`flex-row items-center mb-2`}>
                      ...
                      <Text style={tw`text-orange-400 font-semibold text-sm ml-2`}>Pro Feature</Text>
                    </View>
                  ...
                  <Text style={tw`text-white/70 text-sm leading-5`}>
                    {(() => {
                      ...
                      if (!processedData.combinedTranscriptionData.words || !Array.isArray(processedData.combinedTranscriptionData.words) || processedData.combinedTranscriptionData.words.length === 0) {
                        ...
                        if (processedData.combinedTranscriptionData.text) {
                          ...
                          const totalDuration = processedData.combinedTranscriptionData.audio_duration || 600; // Default 10 minutes
                          ...
                          if (chunkText.length < 20 && chunkStartChar < fullText.length) {
                            chunkText = fullText.slice(chunkStartChar, Math.min(chunkStartChar + 100, fullText.length)).trim();
                          }
                      ...
                      const wordsInChunk = processedData.combinedTranscriptionData.words.filter(word => {
                        if (!word || typeof word.start !== 'number') {
                          return false;
                        }
                        const wordTime = word.start;
                        return wordTime >= selectedPoint.timestamp && wordTime < selectedPoint.timestamp + 3;
                      });
  ...
  const renderChatSection = () => {
    ...
    const handleNavigateToChat = async () => {
      ...
      try {
        ...
        const chunksWithEmbeddings = processedData.timelineData.filter(chunk => 
  ...
  return (
    <MotiView
      ...
      style={[tw`flex-1  pt-12`, shadowStyle]}
    >
      <ScrollView 
      ...
      >
        <View style={tw`flex-row justify-between items-start`}>
          ...
          <TouchableOpacity onPress={onReset} style={tw`bg-white/10 px-3 mb-3 mt-2 py-2 rounded-lg`}>
          ...
          <SummaryStats 
            stats={{
              ...
              results: processedData.timelineData.map(item => ({
                timestamp: item.timestamp,
                prediction: item.prediction,
                confidence: Math.min(1, Math.max(0, parseFloat(item.confidence) > 1 ? parseFloat(item.confidence) / 100 : parseFloat(item.confidence)))
              }))
            }} 
          />
        </MotiView>
            ...
            <GlassCard style={tw`mb-2 rounded-2xl`}>
              ...
              <View style={tw`flex-row justify-between mb-3`}>
                <View style={tw`items-center flex-1`}>
                  ...
                  <Text style={tw`text-white font-bold text-lg`}>{processedData.summaryStats.human}%</Text>
                  <Text style={tw`text-white/60 text-xs`}>{processedData.summaryStats.humanCount || 0} chunks</Text>
                </View>
                ...
                {processedData.summaryStats.mixed > 0 && (
                  <View style={tw`items-center flex-1`}>
                    ...
                    <Text style={tw`text-white font-bold text-lg`}>{processedData.summaryStats.mixed}%</Text>
                    <Text style={tw`text-white/60 text-xs`}>{processedData.summaryStats.mixedCount || 0} chunks</Text>
                  </View>
                )}
                ...
                <View style={tw`items-center flex-1`}>
                  ...
                  <Text style={tw`text-white font-bold text-lg`}>{processedData.summaryStats.ai}%</Text>
                  <Text style={tw`text-white/60 text-xs`}>{processedData.summaryStats.aiCount || 0} chunks</Text>
                </View>
              </View>
              ...
              <View style={tw`h-2 bg-white/20 rounded-full overflow-hidden`}>
                <View style={tw`flex-row h-full`}>
                  <View 
                    style={[
                      tw`bg-green-500`,
                      { width: `${processedData.summaryStats.human}%` }
                    ]} 
                  />
                  {processedData.summaryStats.mixed > 0 && (
                    <View 
                      style={[
                        tw`bg-orange-500`,
                        { width: `${processedData.summaryStats.mixed}%` }
                      ]} 
                    />
                  )}
                  <View 
                    style={[
                      tw`bg-red-700`,
                      { width: `${processedData.summaryStats.ai}%` }
                    ]} 
                  />
                </View>
              </View>
          ...
          >
            <GlassCard style={tw`border border-blue-400/30 bg-blue-500/10`}>
              ...
              <Text style={tw`text-white/90 text-sm mb-3`}>
                Enhanced AI analysis features are active for this audio file, enabling deeper insights and intelligent chat assistance.
              </Text>
              
              <View style={tw`flex-row items-center`}>
                <Ionicons name="checkmark-circle" size={16} color="#22C55E" />
                <Text style={tw`text-white/70 text-sm ml-2`}>
                  Advanced chat analysis ready
                </Text>
              </View>
            </GlassCard>
          </MotiView>
