...
export default function TimelineGrid({ 
  ...
  const processedData = chartData.map((item, index) => {
    ...
    if (item.chunk !== undefined) {
      ...
      prediction = item.prediction || 'unknown';
      probability_ai = typeof item.probability_ai === 'number' ? item.probability_ai : parseFloat(item.probability_ai) || 0;
  ...
  const renderSummaryStats = () => {
    ...
    const aiCount = processedData.filter(d => d.prediction === 'ai' || d.probability_ai > 75).length;
    const humanCount = processedData.filter(d => d.prediction === 'human' || d.probability_ai < 40).length;
    ...
    return (
      <GlassCard style={tw`rounded-2xl`}>
        ...
        <View style={tw`flex-row bg-white/10 rounded-t-lg p-3`}>
          ...
          <Text style={tw`text-white/80 text-sm font-semibold flex-1 text-right`}>Transcription</Text>
        </View>
                ...
                <View style={tw`flex-1`}>
                  <Text style={tw`text-white/70 text-xs text-right`} numberOfLines={2}>
                    {transcriptionData?.words ? 
                      transcriptionData.words
                        .filter(word => word.start >= dataPoint.timestamp && word.start < dataPoint.timestamp + 3)
                        .map(word => word.word)
                        .join(' ')
                        .substring(0, 50) + (transcriptionData.words
                          .filter(word => word.start >= dataPoint.timestamp && word.start < dataPoint.timestamp + 3)
                          .map(word => word.word)
                          .join(' ').length > 50 ? '...' : '')
        ...
        {!hasSubscription && processedData.length > 3 && (
          <View style={tw`mt-4 pt-4 border-t border-white/20`}>
            <Text style={tw`text-orange-400 text-xs text-center`}>
              Upgrade to Pro for full timeline access ({processedData.length - 3} more segments available)
            </Text>
          </View>
        )}
