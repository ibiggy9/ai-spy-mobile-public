...
# Print Deepgram API key (masked for security but checking it exists)
deepgram_api_key = os.getenv('DEEPGRAM_API_KEY')
if deepgram_api_key:
    masked_key = deepgram_api_key[:4] + '*****' + deepgram_api_key[-4:] if len(deepgram_api_key) > 8 else "***"
    print(f"Deepgram API key loaded: {masked_key}")
else:
    print("WARNING: No Deepgram API key found! Transcription will not work.")
...
async def generate_upload_url(request: SignedUrlRequest, authorization: str = Header(None)):
    try:
        ...
        print(f"Initializing storage client with project: {os.getenv('GOOGLE_CLOUD_PROJECT')}")
...
"""
            ...
            if chunk_results:
                ...
                for i, chunk in enumerate(chunk_results[:10]):  # Limit to first 10 chunks to avoid token limits
...
async def security_status():
    """
    Security validation endpoint - should be removed or protected in production
    This endpoint helps validate that security measures are properly configured
    """
    from security_config import SecurityConfig, SECURITY_CHECKLIST
    ...
    return {
        ...
        "test_endpoints_removed": True,  # Since we removed them
        "server_side_subscription_validation": True,
        "recommendation": "Remove this endpoint in production" if validation_result["is_production_ready"] else "Fix security issues before deployment"
    }
